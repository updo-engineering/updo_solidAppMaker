...
const SignInScreen = ({ navigation }) => {
  ...
  async function onFacebookButtonPress() {
    try {
      ...
      console.log(moment(fbdata.expirationTime).toDate());
      if (!fbdata) {
        throw 'Something went wrong obtaining access token';
      }
       const facebookCredential = auth.FacebookAuthProvider.credential(fbdata.accessToken);
       console.log(facebookCredential)
       let fbuserData = auth().signInWithCredential(facebookCredential);
       let fbuser = fbuserData?.user?._user
  ...
  const validateAccount = (loginSource,email,authToken,name,img,phone,countryCode) =>{
    setLoading(true)
    validateUser(phone, countryCode, loginSource, email, authToken).then(response => {
      if (response.ok) {
        setLoading(false)
        if (response.data?.status === true && response.data?.other?.status == 'new') {
          navigation.navigate('SelectionScreen', { phone: phone, countryCode: countryCode, loginSource: loginSource, email: email, socialName: name, socialImg: img,authToken:authToken })
        }
