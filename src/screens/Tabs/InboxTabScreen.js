...
const InboxTabScreen = ({navigation}) => {
    ...
    useEffect(() => {
      ...
      const Item = ({ item }) => (
        <TouchableOpacity onPress={() => navigation.navigate('MessageScreen',{key:item._data.key,chatHeader:item._data.to,toID:item._data.toUid})}  style={{width:'92%', flexDirection:"row", backgroundColor: "white", borderRadius: 16, height: 72, margin: 16,shadowColor: "grey",shadowOpacity: 0.4, elevation: 3, shadowOffset: { width: 0, height: 1 } }}>
                <Image style={{ resizeMode: "cover", width: 44, height: 44, borderRadius: 22, marginHorizontal:12, marginVertical:12,marginVertical:14 }} source={item._data.toProfileImg == '' ? require("../../assets/dummy.png") : {uri:item._data.toProfileImg}}></Image>
